#!/bin/python
# Monitors some sensors and stores data to log file
# Has pushbutton to switch monitoring, pause monitoring, or shutdown
...
def get_temperature(bus_address):
    convert_temperature(bus_address)
    byte_tmsb = bus.read_byte_data(bus_address, 0x11)
    byte_tlsb = bus.read_byte_data(bus_address, 0x12)
...
def convert_temperature(bus_address):
    byte_control = bus.read_byte_data(bus_address, 0x0E)
    if byte_control & 32 == 0:
        bus.write_byte_data(bus_address, 0x0E, byte_control | 32)
    byte_control = bus.read_byte_data(bus_address, 0x0E)
...
while button_status < BUTTON_SHUTDOWN and counter < 1000:
    ...
    if monitor_latch:
        ...
        if sensor_selected == SENSOR_LIGHT or sensor_selected == SENSOR_ALL:
            logging.info("Sensor:LightLevel Value:%s", rc_time(LDR))
