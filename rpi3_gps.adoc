...
Pulse per second (PPS or 1PPS) is signal that accurately repeats once per second. Precision clocks align a PPS signal to the UTC second and converts it to a useful time.
...
Your preferred server should have an *** by it and the PPS server should have a *o*. Additionally, the reach of available servers should eventually get to 377. The offset and the jitter from the PPS source should be very low.
